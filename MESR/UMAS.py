# -*- coding: utf-8 -*-
"""
Created on Fri Aug 30 14:37:54 2024
...
class Mission_Architecture():
    def __init__(self, imLEO, mprop_lander, md_lander, md_lander_bkdn, mp_surface, mprop_cycle):
        self.imLEO = imLEO
...
#%% 2t case
def ARCH_Sizer_MA1(mpL, dvL1, dvL2, LUT):
    Lander = L.Lander_sizer(mpL, dvL1, LUT)
    mp_TV = Lander.mt
    imLEO = Lander.md
    mprop_lander1 = int(Lander.mprop)

    md_lander = Lander.md
    md_lander_bkdn = Lander.dry_mass_breakdown
    mp_surface = mpL
    mprop_lander2 = L.prop(mpL, Lander.md, LUT.Isp, dvL2)
    mprop_cycle = Lander.mprop + 2*(mprop_lander2)
    MA = Mission_Architecture(imLEO, mprop_lander1, md_lander, md_lander_bkdn, mp_surface, mprop_cycle)
    return MA
#%%
def ARCH_Construction_Sizer_MA1(max_pl_LEO, mpL, dvL, dvT, LUT, TUT, LaUT):
    Lander = L.Lander_sizer(mpL, dvL, LUT)
    mp_TV = Lander.md
    TV = T.TV_scaler(mp_TV, dvT, TUT)
    imLEO = Lander.md + TV.md + TV.m_prop
    if Lander.md + TV.md + TV.m_prop < max_pl_LEO:
        constr_launches = 1
    else:
    ...
    imLEO = Lander.md + TV.md + TV.m_prop
    init_md_LEO = Lander.md + TV.md
    init_mprop_LEO = TV.m_prop
...
#%%
#MA% Max mpL case
def Max_ARCH_Sizer(Max_pl_LEO, dvL, dvT, LUT, TUT):
    y1 = [] #total mass
    ...
    return Mission_Architecture(imLEO, Lander_test.mprop, TV_test.m_prop, Lander_test.md, TV_test.md, Lander_test.dry_mass_breakdown, mpL_max, mprop_cycle)
...
MA1 = ARCH_Sizer_MA1(mpL, dvL1, dvL2, LUT)
# CA3 = ARCH_Construction_Sizer_MA1(LaUT.mp, mpL, dvL, dvT, LUT, TUT, LaUT)


# Max_MA3 = Max_ARCH_Sizer(LaUT.mp, dvL, dvT, L_Me, TUT)
# Max_CA3 = 
